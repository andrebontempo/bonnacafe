...
if (typeof window !== 'undefined') {
  ...
  window.BonnaMenu.injectPricesCSS = function(items) {
    var itemsList = (items && Array.isArray(items) && items.length > 0) ? items : BONNA_ITEMS;
    var cssText = '';
    itemsList.forEach(function(item) {
      if (!item || !item.id) return;
      var priceVal = (typeof item.price === 'number') ? item.price : parseFloat(item.price);
      if (isNaN(priceVal)) return;
      var formatted = priceVal.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      cssText += '#_' + item.id + '::before, #_' + item.id + '::after { content: "R$ ' + formatted + '"; }\n';
    });

    var styleElem = document.getElementById('bonna-dynamic-prices-css');
    if (!styleElem) {
      styleElem = document.createElement('style');
      styleElem.id = 'bonna-dynamic-prices-css';
